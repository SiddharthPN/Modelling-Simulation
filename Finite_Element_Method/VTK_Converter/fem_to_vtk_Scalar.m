function fem_to_vtk_Scalar (OutputVTKFileName, NodalCoords, ElemConnect, varargin)

%*****************************************************************************80
%
% FEM_TO_VTK converts an FEM model into a VTK ASCII file.
%
%  Usage:
%
%    fem_to_vtk_Scalar (OutputVTKFileName, NodalCoords, ElemConnect, Values(optional) )
%
...
%    * Values[optional]:  the array of scalar values defined at each node (Size:numNodes X num_fields). 
%
% Licensing:
%
...
%
  ...
  if (nargin==3)
      values=NodalCoords(:,1)'*0.0; value_num=1; 
  elseif (nargin>3)    
...
for field=1:value_num
    fprintf (vtkFile, '<DataArray  type="Float32"  Name="Field%i"  NumberOfComponents="1" format="ascii">\n', field);
    for node=1:node_num
        fprintf (vtkFile, '%f\n', values(field, node));
    end
    fprintf (vtkFile, '</DataArray> \n');
